...
import {
  ...
  getAgentFields,
  type AgentFormData,
} from "@/features/medical/admin/agents";
...
export function AgentForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = agentId ? `/api/medical/agents/${agentId}` : "/api/medical/agents";
    ...
    if (onSuccess) {
      ...
      router.push("/admin/medical/agents?manage=1");
      router.refresh();
    }
  };
  ...
  const formBody = (
    <>
      ...
      <div className={fieldGrid}>
        {getAgentFields(agentFieldNames).map((field) => (
          <FormField
            key={field.name}
            id={field.name}
            name={field.name}
            label={field.label}
            as={field.name === "branch" ? "select" : "input"}
  ...
  const formActions = (
    ...
    >
      ...
      {onCancel ? (
        ...
        <ButtonLink href="/admin/medical/agents?manage=1" variant="secondary" size="sm">
          Cancel
        </ButtonLink>
      )}
    </div>
